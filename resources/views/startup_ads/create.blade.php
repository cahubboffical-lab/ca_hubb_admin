...
@section('title') {{ __('Add :section', ['section' => __($config['label'])]) }} @endsection
...
@section('content')
    <section class="section">
        <form action="{{ route('startup-ads.store', compact('section')) }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="card"><div class="card-body mt-3">
                @include('startup_ads.partials.form', ['startupAd' => $startupAd])
            </div></div>
            <div class="text-end"><button type="submit" class="btn btn-primary">{{ __('Save Ad') }}</button></div>
        </form>
    </section>
@endsection
