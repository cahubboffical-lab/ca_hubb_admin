...
// Scheduler endpoint for cron job with 1-minute lock
Route::get('/run-scheduler', function () {
    if (Cache::has('scheduler_running')) {
        return response()->json(['status' => 'Already processed recently']);
    }

    Cache::put('scheduler_running', true, 60); // lock for 60 seconds

    try {
        // Run scheduler for other scheduled tasks
        Artisan::call('schedule:run');
        $scheduleOutput = Artisan::output();
        
        // Directly process queue jobs (since scheduler might not catch it)
        $queueJobsBefore = DB::table('jobs')->count();
        $queueOutput = '';
        
        if ($queueJobsBefore > 0) {
            try {
                // Process queue with stop-when-empty
                ...
                $queueOutput = 'Queue error: ' . $e->getMessage();
                Log::error('Queue processing error', ['error' => $e->getMessage()]);
            }
        }
        
        $queueJobsAfter = DB::table('jobs')->count();
        $jobsProcessed = $queueJobsBefore - $queueJobsAfter;
        
        return response()->json([
            'status' => 'Scheduler processed',
            'schedule_output' => $scheduleOutput,
            'queue_output' => $queueOutput,
            'pending_jobs_before' => $queueJobsBefore,
            'pending_jobs_after' => $queueJobsAfter,
            'jobs_processed' => $jobsProcessed
        ]);
    } finally {
        Cache::forget('scheduler_running');
    }
});
